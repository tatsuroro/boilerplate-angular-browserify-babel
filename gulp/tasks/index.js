...
module.exports = gulp.task('index', function() {
  return gulp.src(config.paths.src.index)
    ...
    .pipe(gulpif(release,
      replace('<!--stylesVendor-->', '<link href="' + config.filenames.release.stylesVendor + '" rel="stylesheet">'),
      replace('<!--stylesVendor-->', '<link href="' + config.filenames.build.stylesVendor + '" rel="stylesheet">')
    ))
    .pipe(gulpif(release,
